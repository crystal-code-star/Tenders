...
function BPItemsModal({ tenderReference, onClose }) {
  ...
  if (loading) return (<div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0F172A]/40 backdrop-blur-sm"><div className="bg-white rounded-3xl shadow-2xl p-8 flex flex-col items-center gap-4"><Loader size={32} className="text-[#2563EB] animate-spin" /><p className="text-sm font-medium text-[#475569]">Loading BP items...</p></div></div>);
  if (error) return (<div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0F172A]/40 backdrop-blur-sm"><div className="bg-white rounded-3xl shadow-2xl p-8 max-w-md mx-4"><div className="flex items-center gap-3 mb-4"><div className="w-10 h-10 rounded-2xl bg-[#FEE2E2] flex items-center justify-center"><AlertTriangle size={20} className="text-[#DC2626]" /></div><div><h3 className="font-semibold text-[#0F172A]">Error Loading BP Data</h3><p className="text-xs text-[#64748B]">{error}</p></div></div><button onClick={onClose} className="w-full py-3 bg-[#111827] text-white rounded-2xl text-sm font-medium hover:bg-[#1E293B] transition-all">Close</button></div></div>);
  return (
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-5xl mx-4 border border-[#E2E8F0]">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-[#64748B]">{totalItems} items • Total HT: {(summary.total_ht || 0).toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} DHS</p>
              </div>
            </div>
            <button onClick={onClose} className="p-2 text-[#64748B] hover:text-[#0F172A] hover:bg-[#F8FAFC] rounded-xl transition-colors">
              <X size={20} />
        ...
        <div className="p-6 grid grid-cols-4 gap-4">
          ...
          <div className="bg-[#FEF9C3]/50 rounded-2xl p-4 border border-[#FDE68A]">
            ...
            <p className="text-lg font-semibold text-[#CA8A04] cw-mono">{(summary.total_ht || 0).toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
          </div>
          <div className="bg-[#F1F5F9] rounded-2xl p-4 border border-[#CBD5E1]">
            ...
            <p className="text-lg font-semibold text-[#475569] cw-mono">{(summary.total_qty || 0).toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
          </div>
          <div className="bg-[#DCFCE7] rounded-2xl p-4 border border-[#BBF7D0]">
            ...
            <p className="text-lg font-semibold text-[#16A34A] cw-mono">{(summary.avg_price || 0).toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
          </div>
        </div>
              ...
              {displayItems.map((item, index) => (
                <React.Fragment key={item.id || index}>
                  <tr className="hover:bg-[#F8FAFC]/50 transition-colors group">
                    ...
                    <td className="px-4 py-3 text-xs text-[#0F172A] max-w-xs"><div className="truncate" title={item.designation}>{item.designation || '—'}</div></td>
                    <td className="px-4 py-3 text-center text-xs text-[#475569]">{item.unite || '—'}</td>
                    <td className="px-4 py-3 text-right cw-mono text-xs text-[#0F172A]">{item.quantite ? parseFloat(item.quantite).toLocaleString('fr-FR') : '—'}</td>
                    <td className="px-4 py-3 text-right cw-mono text-xs font-medium text-[#2563EB]">{item.prix_unitaire_ht ? parseFloat(item.prix_unitaire_ht).toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '—'}</td>
                    <td className="px-4 py-3 text-right cw-mono text-xs font-semibold text-[#0F172A]">{item.total_ht ? parseFloat(item.total_ht).toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '—'}</td>
...
function TenderCard({ item, active, onClick, onQualify }) {
  ...
  return (
    <button onClick={onClick}
      ...
      style={!active ? { borderColor: isUnseen ? '#CBD5E1' : '#E2E8F0' } : {}}>
            ...
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex items-center gap-3 flex-shrink-0">
                <div className="flex items-center gap-2">
                  ...
                  <div className="bg-[#F8FAFC] rounded-xl px-3 py-1.5 min-w-[85px]">
                    ...
                    {deadline && (
                      <div className={`flex items-center gap-1 ${isExpired ? 'opacity-50' : ''}`}>
                        ...
                        <span className={`cw-mono text-[11px] font-semibold ${
                          isUrgent ? 'text-[#DC2626]' : isExpired ? 'text-[#94A3B8] line-through' : 'text-[#475569]'
                        }`}>
                  ...
                  {deadline && daysLeft !== null && !isExpired && (
                    <div className={`rounded-xl px-3 py-1.5 min-w-[55px] text-center ${
                      ...
                      <div className={`cw-mono text-[15px] font-bold leading-tight ${
                        isUrgent ? 'text-[#DC2626]' : daysLeft <= 3 ? 'text-[#CA8A04]' : 'text-[#475569]'
                      }`}>
                        {daysLeft}
                      </div>
                      <div className={`text-[9px] font-medium uppercase tracking-wide ${
                        isUrgent ? 'text-[#DC2626]' : daysLeft <= 3 ? 'text-[#CA8A04]' : 'text-[#94A3B8]'
                      }`}>
                        jour{daysLeft > 1 ? 's' : ''}
                      </div>
                    </div>
                ...
                <ChevronRight size={18} className={`transition-all duration-200 ${
                  active ? 'text-[#2563EB] translate-x-1' : 'text-[#CBD5E1] group-hover:text-[#2563EB] group-hover:translate-x-1'
                }`} />
              </div>
            </div>
          </div>
          ...
          <div className="flex items-start gap-4 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {isExpired && (
                  <span className="inline-flex items-center gap-1 px-3 py-1 rounded-xl bg-[#F1F5F9] text-[11px] font-semibold text-[#94A3B8]">
                    Expired
                  </span>
                )}
      ...
      <div className="p-6 space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <div className="bg-white rounded-2xl border border-[#E2E8F0] p-4 shadow-sm hover:shadow-md transition-shadow duration-200 col-span-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-[#0F172A] truncate cw-mono">
                        {item.date_limite_remise_plis ? fmtDate(item.date_limite_remise_plis) : '—'}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <InfoCard icon={Laptop} label="Type de réponse" value={item.reponse_electronique_obligatoire === true ? 'Électronique' : item.reponse_electronique_obligatoire === false ? 'Physique' : null} color="#475569" />
          </div>
          {otherFields.length > 0 && (
            <div className="bg-white rounded-2xl border border-[#E2E8F0] shadow-sm overflow-hidden mb-4">
              ...
              <div className="p-2">
                {otherFields.map((f, i) => <DetailField key={i} {...f} />)}
              </div>
            </div>
          )}
          {administrativeFields.length > 0 && (
            <div className="bg-white rounded-2xl border border-[#E2E8F0] shadow-sm overflow-hidden mb-4">
              ...
              <div className="p-2">
                {administrativeFields.map((f, i) => <DetailField key={i} {...f} />)}
              </div>
            </div>
          )}
          {financialFields.length > 0 && (
            <div className="bg-white rounded-2xl border border-[#E2E8F0] shadow-sm overflow-hidden mb-4">
              ...
              <div className="p-2">
                {financialFields.map((f, i) => <DetailField key={i} {...f} />)}
              </div>
            </div>
          )}
          {technicalFields.length > 0 && (
            <div className="bg-white rounded-2xl border border-[#E2E8F0] shadow-sm overflow-hidden mb-4">
              ...
              <div className="p-2">
                {technicalFields.map((f, i) => <DetailField key={i} {...f} />)}
              </div>
            </div>
          )}
        </div>
    ...
    <div className="cw-theme min-h-screen">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <button onClick={() => setAutoRefresh(r => !r)} className={`flex items-center gap-2 px-4 py-2 rounded-2xl text-xs font-medium transition-all duration-200 active:scale-95 ${
                    autoRefresh 
                      ? 'bg-[#EFF6FF] text-[#2563EB] border border-[#2563EB]/20 shadow-sm' 
                    ...
                    {autoRefresh ? (
                      <>
                        ...
                        <span className="flex h-1.5 w-1.5">
                          <span className="animate-ping absolute inline-flex h-1.5 w-1.5 rounded-full bg-[#2563EB] opacity-75"></span>
              ...
              {scanning && (
                <div className="mt-4 flex items-center gap-3 px-4 py-3 bg-[#EFF6FF] border border-[#2563EB]/20 rounded-2xl">
                  ...
                  <div className="flex-1">
                    <p className="text-xs font-medium text-[#0F172A]">Scan in progress…</p>
                    <p className="text-[11px] text-[#64748B]">New tenders will appear automatically</p>
                  </div>
                  <span className="text-[11px] text-[#94A3B8] cw-mono">
                    {lastScan ? `Last: ${lastScan.toLocaleTimeString()}` : 'Scanning...'}
                  </span>
                </div>
              )}
            </div>
        ...
        <div className="sticky top-0 z-20 bg-[#F8FAFC]/95 backdrop-blur-sm pb-4 -mt-2 pt-2">
          <div className="bg-white rounded-3xl border border-[#E2E8F0] shadow-sm p-4">
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1 min-w-0">
                ...
                <input 
                  ...
                  placeholder="Search by title, buyer, location, reference..." 
                  className="w-full pl-11 pr-4 py-3 bg-[#F8FAFC] border border-[#E2E8F0] rounded-2xl text-sm text-[#0F172A] placeholder-[#94A3B8] focus:ring-2 focus:ring-[#2563EB]/20 focus:border-[#2563EB] focus:bg-white outline-none transition-all" 
                />
              </div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <FilterPill active={useKeywordFilter} onClick={() => { if (!useKeywordFilter) setShowKeywordManager(true); else setUseKeywordFilter(false); }} icon={<SlidersHorizontal size={11} />}>
                  Mots-clés
                  {useKeywordFilter && activeKeywordCount > 0 && (
                    <span className="inline-flex items-center justify-center w-4 h-4 rounded-full bg-[#2563EB] text-white text-[9px] font-semibold">
                      {activeKeywordCount}
                    </span>
                  )}
                </FilterPill>
            ...
            {qualificationFilters.length > 0 && (
              <div className="mt-3 flex items-center gap-2 px-4 py-3 bg-[#EFF6FF] border border-[#2563EB]/20 rounded-2xl">
                <Filter size={12} className="text-[#2563EB] flex-shrink-0" />
                <div className="flex items-center gap-1 flex-wrap flex-1">
                  <span className="text-[11px] font-medium text-[#0F172A]">Statut :</span>
                  {qualificationFilters.map(f => (
                    <span key={f} className={`px-2 py-0.5 rounded-lg text-[11px] font-medium border ${
                      f === 'preselected' ? 'bg-amber-50 text-amber-700 border-amber-300' : 'bg-emerald-50 text-emerald-700 border-emerald-300'
                    }`}>
                      {f === 'preselected' ? 'Présélectionnés' : 'Qualifiés'}
                    </span>
                  ))}
                </div>
              </div>
            ...
            <div className="mt-3 flex items-center justify-between text-[11px] text-[#94A3B8]">
              <span>
                <span className="font-semibold text-[#0F172A] cw-mono">{current.length}</span> appel{current.length !== 1 ? 's' : ''} d'offre{current.length !== 1 ? 's' : ''} trouvé{current.length !== 1 ? 's' : ''}
              </span>
              {lastScan && (
                <span className="cw-mono">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-32 gap-4">
            <div className="relative">
              <div className="w-16 h-16 rounded-3xl bg-white border border-[#E2E8F0] shadow-sm flex items-center justify-center">
            ...
            <p className="text-xs text-[#94A3B8]">Récupération des dernières opportunités</p>
          </div>
        ) : current.length === 0 ? (
          <div className="bg-white rounded-3xl border border-[#E2E8F0] shadow-sm">
            <div className="flex flex-col items-center text-center py-20 px-4">
              <div className="w-16 h-16 rounded-3xl bg-[#F8FAFC] flex items-center justify-center mb-4">
                <Globe size={28} className="text-[#94A3B8]" />
              </div>
              ...
              <p className="text-sm text-[#475569] max-w-md mb-6">
                {tenders.length === 0 
                  ? "Vous n'avez pas encore découvert d'appels d'offres. Lancez votre premier scan." 
                  : 'Aucun appel d\'offre ne correspond à vos filtres.'
                }
              </p>
              {tenders.length === 0 ? (
                <button onClick={handleScan} disabled={scanning} className="flex items-center gap-2 px-6 py-3 rounded-2xl font-medium text-sm text-white bg-[#111827] hover:bg-[#1E293B] disabled:opacity-50 transition-all duration-200 shadow-sm active:scale-95">
                  <Zap size={14} /> Lancer le scan
                </button>
              ) : (
                <button onClick={clearAllFilters} className="flex items-center gap-2 px-6 py-3 rounded-2xl font-medium text-sm text-[#0F172A] bg-white border border-[#E2E8F0] hover:bg-[#F8FAFC] transition-all duration-200 shadow-sm">
                  Effacer les filtres
                </button>
              )}
            </div>
          ...
          <div className="flex flex-col gap-3">
            {current.map(item => (
              <TenderCard 
                ...
                item={item} 
                active={selected && (selected.reference || selected.id) === (item.reference || item.id)} 
                onClick={() => handleSelectTender(item)} 
      ...
      <KeywordManager 
        ...
        onClose={() => { 
          ...
          if (keywords.filter(k => k.is_active).length === 0) setUseKeywordFilter(false); 
        }} 
        keywords={keywords} 
        onAdd={async (keyword) => { 
          await addKeyword(keyword); 
          setUseKeywordFilter(true); 
        }} 
